define("./workbox-5b2c88fa.js",['exports'], function (exports) { 'use strict';
    ...
    */
    ...
    async function copyResponse(response, modifier) {
      ...
      if (origin !== self.location.origin) {
        throw new WorkboxError$1('cross-origin-copy-response', {
          origin
        });
      }
    ...
    */
    ...
    const isInstance$2 = (object, expectedClass, details) => {
      if (!(object instanceof expectedClass)) {
        details['expectedClass'] = expectedClass;
    ...
    const isArrayOfClass$2 = (value, expectedClass, details) => {
    ...
    */
    ...
    async function executeQuotaErrorCallbacks() {
      {
        logger$2.log(`About to run ${quotaErrorCallbacks.size} ` + `callbacks to clean up caches.`);
      }
      ...
      for (const callback of quotaErrorCallbacks) {
        ...
        {
          logger$2.log(callback, 'is complete.');
        }
      }

      {
        logger$2.log('Finished running callbacks.');
      }
    }
    ...
    */
    ...
    class StrategyHandler {
      ...
      constructor(strategy, options) {
        ...
        {
          finalAssertExports$2.isInstance(options.event, ExtendableEvent, {
        ...
        return this.waitUntil((async () => {
          ...
          if (request.mode === 'navigate' && event instanceof FetchEvent && event.preloadResponse) {
            ...
            if (possiblePreloadResponse) {
              {
                logger$2.log(`Using a preloaded navigation response for ` + `'${getFriendlyURL$2(request.url)}'`);
              }
          ...
          try {
            ...
            throw new WorkboxError$2('plugin-error-request-will-fetch', {
              thrownError: err
            });
          ...
          try {
            ...
            if ("development" !== 'production') {
              logger$2.debug(`Network request for ` + `'${getFriendlyURL$2(request.url)}' returned a response with ` + `status '${fetchResponse.status}'.`);
            }
            ...
            {
              logger$2.error(`Network request for ` + `'${getFriendlyURL$2(request.url)}' threw an error.`, error);
      ...
      cacheMatch(key) {
        return this.waitUntil((async () => {
          ...
          {
            if (cachedResponse) {
              logger$2.debug(`Found a cached response in '${cacheName}'.`);
            } else {
              logger$2.debug(`No cached response found in '${cacheName}'.`);
            }
          }
      ...
      async cachePut(key, response) {
...

        {
          if (effectiveRequest.method && effectiveRequest.method !== 'GET') {
            throw new WorkboxError$2('attempt-to-cache-non-get-request', {
              url: getFriendlyURL$2(effectiveRequest.url),
        ...
        if (!response) {
          {
            logger$2.error(`Cannot cache non-existent response for ` + `'${getFriendlyURL$2(effectiveRequest.url)}'.`);
          }

          throw new WorkboxError$2('cache-put-with-no-response', {
            url: getFriendlyURL$2(effectiveRequest.url)
          });
        }
        ...
        if (!responseToCache) {
          {
            logger$2.debug(`Response '${getFriendlyURL$2(effectiveRequest.url)}' ` + `will not be cached.`, responseToCache);
          }
        ...
        {
          logger$2.debug(`Updating the '${cacheName}' cache with a new Response ` + `for ${getFriendlyURL$2(effectiveRequest.url)}.`);
        }
            ...
            if (responseToCache) {
              if (responseToCache.status !== 200) {
                if (responseToCache.status === 0) {
                  logger$2.warn(`The response for '${this.request.url}' ` + `is an opaque response. The caching strategy that you're ` + `using will not cache opaque responses by default.`);
                } else {
                  logger$2.debug(`The response for '${this.request.url}' ` + `returned a status code of '${response.status}' and won't ` + `be cached as a result.`);
                }
              }
            }
          }
      ...
      async _getResponse(handler, request, event) {
        ...
        try {
          ...
          if (!response || response.type === 'error') {
            throw new WorkboxError$2('no-response', {
              url: request.url
            });
          }
        } catch (error) {
          ...
          if (!response) {
            ...
            logger$2.log(`While responding to '${getFriendlyURL$2(request.url)}', ` + `an ${error} error occurred. Using a fallback response provided by ` + `a handlerDidError plugin.`);
          }
        }
    ...
    class PrecacheStrategy extends Strategy {
      ...
      async _handleFetch(request, handler) {
        ...
        if (this._fallbackToNetwork) {
          {
            logger$1.warn(`The precached response for ` + `${getFriendlyURL$1(request.url)} in ${this.cacheName} was not ` + `found. Falling back to the network instead.`);
          }
...

        {
          ...
          logger$1.groupCollapsed(`Precaching is responding to: ` + getFriendlyURL$1(request.url));
          logger$1.log(`Serving the precached url: ${getFriendlyURL$1(cacheKey.url)}`);
    ...
    class PrecacheRoute extends Route {
      ...
      constructor(precacheController, options) {
        const match = ({
          ...
          {
            logger$1.debug(`Precaching did not find a match for ` + getFriendlyURL$1(request.url));
          }
